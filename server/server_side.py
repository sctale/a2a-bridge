#!/usr/bin/env python3
"""
Agent A A2A Server — v2.2.1
Receives task delegations from Agent B, processes with AI, returns results.
脱敏版：环境变量控制所有敏感配置，纯 httpx 直调 AI（无 Hermes 依赖）
"""
import asyncio
import json
import logging
...
logger = logging.getLogger("agent_a_a2a")

# ===== FastAPI App =====
app = FastAPI(
    title="Agent A A2A Server",
    version="2.2.1",
...
class IdempotencyStore:
    """轻量幂等存储：task_id 查重 + TTL 缓存 + failed 允许重试"""

    TTL_HOURS = 24

    def __init__(self, db_path: str = "/tmp/agent_a_a2a_idempotency.db"):
...
_idempotency = IdempotencyStore(
    db_path=os.environ.get("A2A_IDEMPOTENCY_DB", "/tmp/agent_a_a2a_idempotency.db")
)
...
class SessionStore:
    ...
    def __init__(self, db_path: str = "/tmp/agent_a_a2a_sessions.db"):
        self._lock = threading.Lock()
        self._db = sqlite3.connect(db_path, timeout=5, check_same_thread=False)
        self._db.execute("""
...
async def call_ai(system_prompt: str, instruction: str, session_id: str = None, task_type: str = "chat") -> str:
    ...
    # 尝试从 .env 文件读取（支持 docker 部署时挂载 .env）
...
async def health():
    return {
        "status": "ok",
        "agent": os.environ.get("AGENT_NAME", "agent-a"),
...
async def create_task(request: Dict[str, Any]):
    try:
        ...
        if to_agent and to_agent != os.environ.get("AGENT_NAME", "agent-a"):
            return JSONResponse(
                status_code=400,
                content=make_response(
                    task_id=task_id,
        ...
        # ===== 构建 system prompt =====
        system_prompt = f"你是 Agent A，一个 AI 助手。回答简洁直接，一针见血。"
        if context:
