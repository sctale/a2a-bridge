# A2A Bridge — 通用双向 Agent 通信框架 v1.3.0

[![Version](https://img.shields.io/badge/version-v1.3.0-blue)](https://github.com/sctale/a2a-bridge/releases)
...
| GET | `/tasks` | 列出最近任务（支持 `?status=` 过滤，`?limit=`/`?offset=` 分页） |
| POST | `/send` | 主动向对端发送任务（`sync` 参数默认 `false`，立即返回 `queued`） |
...
```bash
# 全部最近 20 条
curl http://localhost:8643/tasks

# 只看失败的
curl 'http://localhost:8643/tasks?status=failed'

# 分页：跳过前 20 条，取 10 条
curl 'http://localhost:8643/tasks?limit=10&offset=20'
```
  ...
  "tasks": [
    {
      "task_id": "...",
      "status": "completed",
      "created_at": "2026-06-01T17:20:14",
      "completed_at": "2026-06-01T17:20:14",
      "result_preview": "pong"
    }
  ]
}
...
| 值 | 行为 |
|---|------|
| `sync=false`（默认） | 立即返回 `{"status":"queued", "task_id":"..."}`，不阻塞 caller |
| `sync=true` | 同步等待对端处理完返回完整结果（可能阻塞 120s） |

**v1.3.0 起默认改 `false`**：caller 不被阻塞，结果通过 `GET /a2a/{task_id}` 或 `GET /tasks` 查询。

---

## 健康检查脚本

`scripts/healthcheck.sh` — 5 行命令检测节点是否健康：

```bash
/opt/data/github-a2a/scripts/healthcheck.sh 8643
# [OK] port=8643 name=hermes-nas peer=http://192.168.20.252:8644
# exit 0=健康，1=不健康
```
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v1.3.0** | 2026-06-01 | `/tasks` 列表端点（status 过滤+分页）；`/send` 默认 `sync=false`（不阻塞 caller）；`requirements.txt` 锁版本；新增 `scripts/healthcheck.sh` 看门狗脚本 |
